...
fetch_record_id <- function(resident, coach_data, redcap_uri, token) {
  ...
  if (!"name" %in% colnames(data) || nrow(data) == 0) {
    message("⚠️ No name column found in REDCap data, or no data available.")
    return(NULL)
  }
  ...
  if (length(record_id) > 0) {
    record_id <- record_id[1]  # 🚩 Only take the first one, if multiples exist
    message("✅ Fetched record_id from REDCap: ", record_id)
    return(record_id)
  }
  
  message("⚠️ No record_id found for resident: ", resident)
  return(NULL)
}
...
generate_new_instance <- function(record_id, instrument_name, coach_data, redcap_uri, token) {
  ...
  ## 🚩 1️⃣ Check for existing instances in coach_data (local cache)
  if (!is.null(coach_data)) {
    ...
    if (length(local_data) > 0) {
      ...
      message("⚠️ No instances found in coach_data. Falling back to API call.")
    }
  } else {
    message("⚠️ coach_data is NULL. Falling back to API call.")
  }
  
  ## 🚩 2️⃣ Fallback to pull from API if coach_data is not available or empty
  data <- tryCatch({
    REDCapR::redcap_read_oneshot(
      redcap_uri = redcap_uri, 
      token = token, 
    ...
    message("⚠️ Error pulling REDCap data: ", e$message)
    return(NULL)
  })
...
fetch_existing_data <- function(record_id, period, redcap_uri, token) {
  data <- tryCatch({
    ...
    message("⚠️ Error in redcap_read_oneshot: ", e$message)
    return(data.frame())  # Return an empty data frame instead of NULL
  })
  
  if (!is.data.frame(data)) {
    message("⚠️ No data returned. Returning empty dataframe.")
    return(data.frame())
  }
